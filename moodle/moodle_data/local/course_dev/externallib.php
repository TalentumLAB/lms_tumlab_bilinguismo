...
class local_course_dev_external extends external_api {
    ...
    public static function insertRecord() {
        ...
        $Message['message']=$get_global_c_progress;
        $Messages[]=$Message;
        return $Messages;
    }
                 ...
                 /** 3. obtener la calificacion final del curso por usuario */
                     ...
                     foreach($grade_point_average_grades as $grade_point_average_gdb){
                         ...
                         foreach($grade_point_average_userGrade as $grade_point_average_ugdb){
                             ...
                             $grade_point_average_getUserIdSimat ="SELECT id FROM simats WHERE number_document=".$grade_point_average_userDocument[1]."";
                             if ($grade_point_average_resultUserIdSimat = $conn->query($grade_point_average_getUserIdSimat)) {
                                 while($grade_point_average_userIdSimat = $grade_point_average_resultUserIdSimat->fetch_object()){
                                     ...
                                     $grade_point_average_getUserIfExist ="SELECT user_id FROM user_indicators WHERE indicator_id=5 AND user_id=".$grade_point_average_userIdSimat->id."";
                                     ...
                                     if ($grade_point_average_resultUserIfExist = $conn->query($grade_point_average_getUserIfExist)) {
                                         ...
                                         if($grade_point_average_resultUserIfExist->num_rows == 0){
                                             $grade_point_average_insertLastAccess = "INSERT INTO user_indicators (`user_id`, `indicator_id`, `calc_indicator`) VALUES(".$grade_point_average_userIdSimat->id.",5,'".$grade_point_average_gdb->finalgrade."')";
                                             ...
                                             if ($conn->query($grade_point_average_insertLastAccess) === TRUE) {
                                                 $grade_point_average_Message['message']= "Se inserto el indicador de Promedio de notas por curso";
                                             } else {
                                                 $grade_point_average_Message['message']="Error en la insercion";
                                             }
                                         }
                                         else{
                                             ...
                                             while($grade_point_average_userIfExist = $grade_point_average_resultUserIfExist->fetch_object()){
                                                 ...
                                                 if($grade_point_average_userIfExist->user_id){
                                                     ...
                                                     $grade_point_average_updateLastAccess = "UPDATE user_indicators SET  `calc_indicator`= '".$grade_point_average_gdb->finalgrade."' WHERE `user_id`=".$grade_point_average_userIdSimat->id." AND indicator_id=5";
                                                     $grade_point_average_Messages = array();
                                                     $grade_point_average_Message = array();
                                                     if ($conn->query($grade_point_average_updateLastAccess) === TRUE) {
                                                         $grade_point_average_Message['message']= "Se Actualizo el indicador de last login";
                                                     } else {
                                                         $grade_point_average_Message['message']="Error en la actualizacion";
                                                     }
                                                 }
                                                 else{
                                                     $grade_point_average_insertLastAccess = "INSERT INTO user_indicators (`user_id`, `indicator_id`, `calc_indicator`) VALUES(".$grade_point_average_userIdSimat->id.",5,'".$grade_point_average_gdb->finalgrade."')";
                                 ...
                                 $Message['message']="Ocurrio un error encontrando el usuario";
                                 return $Messages;
                             }
                         }
                     }
                        ...
                        foreach($last_signin_userLastAccess as $last_signin_uladb){
                            ...
                            if($last_signin_uladb->last_signin_lastaccess == 0){
                                ...
                                foreach($last_signin_userToInsertDate as $last_signin_utiddb){
                                    ...
                                    $last_signin_getUserIdSimatToInsertDate ="SELECT id FROM simats WHERE number_document=".$last_signin_userDocumentToInsertDate[1]."";
                                    ...
                                    if ($last_signin_resultUserIdSimatToInsertDate = $conn->query($last_signin_getUserIdSimatToInsertDate)) {
                                        ...
                                        $last_signin_getUserIfExist ="SELECT user_id FROM user_indicators WHERE indicator_id=2 AND user_id=".$last_signin_userIdSimatToInsertDate->id."";

                                        if ($last_signin_resultUserIfExist = $conn->query($last_signin_getUserIfExist)) {

                                            if($last_signin_resultUserIfExist->num_rows == 0){
                                                $last_signin_insertLastAccess = "INSERT INTO user_indicators (`user_id`, `indicator_id`, `calc_indicator`) VALUES(".$last_signin_userIdSimatToInsertDate->id.",2,'".$last_signin_dateToIsertIndicator."')";
                                                $last_signin_Messages = array();
                                                $last_signin_Message = array();
                                                if ($conn->query($last_signin_insertLastAccess) === TRUE) {
                                                    $last_signin_Message['message']= "Se inserto el indicador de Promedio de notas por curso";
                                                } else {
                                                    $last_signin_Message['message']="Error en la insercion";
                                                }
                                            }
                                            else{
                                                ...
                                                while($last_signin_userIfExist = $last_signin_resultUserIfExist->fetch_object()){
                                                    ...
                                                    if($last_signin_userIfExist->user_id){
                                                        ...
                                                        $last_signin_updateLastAccess = "UPDATE user_indicators SET  `calc_indicator`= '".$last_signin_dateToIsertIndicator."' WHERE `user_id`=".$last_signin_userIdSimatToInsertDate->id." AND indicator_id=2";
                                                        $last_signin_Messages = array();
                                                        $last_signin_Message = array();
                                                        if ($conn->query($last_signin_updateLastAccess) === TRUE) {
                                                            $last_signin_Message['message']= "Se Actualizo el indicador de last login";
                                                        } else {
                                                            $last_signin_Message['message']="Error en la actualizacion";
                                                        }
                                                    }
                                                    else{
                                                        $last_signin_insertLastAccess = "INSERT INTO user_indicators (`user_id`, `indicator_id`, `calc_indicator`) VALUES(".$last_signin_userIdSimatToInsertDate->id.",2,'".$last_signin_dateToIsertIndicator."')";
        ...
        foreach($global_progress_courses as $global_progress_cdb){
            ...
            foreach($global_progress_userId as $global_progress_uiddb){
                ...
                foreach($global_progress_userToInsertDate as $global_progress_utiddb){
                    ...
                    $global_progress_getUserIdSimatToInsertDate ="SELECT id FROM simats WHERE number_document=".$global_progress_userDocumentToInsertDate[1]."";
                    ...
                    if ($global_progress_resultUserIdSimatToInsertDate = $conn->query($global_progress_getUserIdSimatToInsertDate)) {
                        ...
                        while($global_progress_userIdSimatToInsertDate = $global_progress_resultUserIdSimatToInsertDate->fetch_object()){
                            $global_progress_getUserIfExist ="SELECT user_id FROM user_indicators WHERE user_id=$global_progress_userIdSimatToInsertDate->id AND indicator_id = 1";
                                    if ($global_progress_resultCourseIfExist = $conn->query($global_progress_getUserIfExist)) {
                                            if($global_progress_resultCourseIfExist->num_rows == 0){
                                                ...
                                                $global_progress_insertNewCantOfActivities = "INSERT INTO `user_indicators` (`user_id`, `indicator_id`,`calc_indicator`) VALUES(".$global_progress_userIdSimatToInsertDate->id.",1,'".$global_progress_progress."')";
                                                ...
                                                while($global_progress_courseIfExist = $global_progress_resultCourseIfExist->fetch_object()){  
                                                    ...
                                                    if($global_progress_courseIfExist->user_id){
                                                        ...
                                                        $global_progress_updateNewCantOfActivities = "UPDATE user_indicators SET  `user_id`= ".$global_progress_userIdSimatToInsertDate->id.",`calc_indicator`='".$global_progress_progress."' WHERE `user_id`=".$global_progress_userIdSimatToInsertDate->id." AND indicator_id = 1";
                                                        $global_progress_Messages = array();
                                                        $global_progress_Message = array();
    ...
    public static function get_global_c_progress(){
        ...
        foreach($courseGeneral_userId as $courseGeneral_uiddb){
            ...
            foreach($global_progress_userToInsertDate as $global_progress_utiddb){
                ...
                $courseGeneral_getCourseIfExist ="SELECT id FROM simats WHERE number_document=".$global_progress_userDocumentToInsertDate[1]."";
                ...
                if ($courseGeneral_resultCourseIfExist = $conn->query($courseGeneral_getCourseIfExist)) {
                    if($courseGeneral_resultCourseIfExist->num_rows == 0){
                        $courseGeneral_insertNewCourse = "INSERT INTO user_indicators (`user_id`,`indicator_id`, `calc_indicator`) VALUES(".$courseGeneral_userId.",9,'".$progress_count/$cant_courses."')";
                        $courseGeneral_Messages = array();
                        $courseGeneral_Message = array();
                        if ($conn->query($courseGeneral_insertNewCourse) === TRUE) {
                            $courseGeneral_Message['message']= "Se inserto el progreso general del curso";
                        } else {
                            $courseGeneral_Message['message']="Error en la insercion";
                        }
                    }
                    else{
                        while($courseGeneral_courseIfExist = $courseGeneral_resultCourseIfExist->fetch_object()){  
                            ...
                            if($courseGeneral_courseIfExist->id_course){
                                ...
                                $courseGeneral_updateNewCourse = "UPDATE user_indicators SET  `user_id`= '".$courseGeneral_userId."',`calc_indicator`='".$progress_count/$cant_courses."' WHERE `user_id`=".$courseGeneral_userId." AND indicator_id = 9";
                                $courseGeneral_Messages = array();
                                $courseGeneral_Message = array();
                                if ($conn->query($courseGeneral_updateNewCourse) === TRUE) {
                                $courseGeneral_Message['message']= "Se Actualizo el progreso general del curso";
                                } else {
                                $courseGeneral_Message['message']="Error en la actualizacion";
                                }
                            }
                            else{
                                    $courseGeneral_insertNewCourse = "INSERT INTO user_indicators (`user_id`,`indicator_id`, `calc_indicator`) VALUES(".$courseGeneral_userId.",9,'".$progress_count/$cant_courses."')";
                                    ...
                                    $courseGeneral_Message = array();
                                    if ($conn->query($courseGeneral_insertNewCourse) === TRUE) {
                                        $courseGeneral_Message['message']= "Se inserto el el progreso general del curso";
                                    } else {
                                        $courseGeneral_Message['message']="Error en la insercion";
                                    }
                            }       
