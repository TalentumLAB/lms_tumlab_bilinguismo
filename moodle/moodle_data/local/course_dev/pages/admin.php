...
/**
 * Determina si se van a insertar usuarios
 */

echo html_writer::tag('hr','');

echo html_writer::tag('h2',"Cargar estudiantes");
echo html_writer::tag('p',"Esta opcion es para seleccionar la sede de la cual se quiere cargar los estudiantes");
...
if(isset($_POST['buttonLoadStudents'])) {
    ...
    if ($res_usersSimatHeadquarters) {
        while ($row_uSH = $res_usersSimatHeadquarters->fetch_assoc()) {
            ...
            */
            $usersSimatHeadquarters_uSimat = "SELECT simat_id FROM headquarters_has_grade_has_group_has_simat WHERE headquarters_has_grade_has_group_id =".$row_uSH['id'];
            ...
            if ($res_usersSimatHeadquarters_uSimat) {
                while ($row_uSHUS = $res_usersSimatHeadquarters_uSimat->fetch_assoc()) {
                     ...
                     * get users
                     */
                    $users = "SELECT id, CONVERT(name USING utf8) as name, CONVERT(second_name USING utf8) as second_name, CONVERT(last_name USING utf8) as last_name, CONVERT(second_surname USING utf8) as second_surname FROM simats WHERE id=".$row_uSHUS['simat_id'];
...
function createUsers($users, $conn){
    ...
    foreach ($users as $user) {
        ...
        $username = strtolower($user['name']).'.'.mb_strtolower($user['last_name'],'UTF-8');
        ...
        */
        ...
        if(isset($clone->debuginfo)){
            ...
            $sqlUsersCourses = "SELECT g.name as gradeName, hgs.simat_id as studentId FROM grades as g JOIN headquarters_has_grade_has_group as hg JOIN headquarters_has_grade_has_group_has_simat as hgs WHERE (g.id = hg.grade_id AND hgs.headquarters_has_grade_has_group_id = hg.id) AND hgs.simat_id =". $userId;
            $res_sqlUsersCourses= $conn->query($sqlUsersCourses);
            
            $courseName;
            foreach($res_sqlUsersCourses as $usercourse){
                $courseName = $usercourse['gradeName'];
            }
            ...
            foreach($res_userExist as $userE){
                ...
                */
                ...
                $res_check = check_enrol($userExist_id, strtolower($courseName));  //descomentar esta opcion cuando se tenga los cursos para poder ejecutar la matriculacion
            }
            $countedBad += 1;
        }else{
            ...
            * Enroll usesr into a course
            */
            check_enrol($clone[0]->id, strtolower($courseName)); //descomentar esta opcion cuando se tenga los cursos para poder ejecutar la matriculacion
        }
    ...
    \core\notification::Error('Total usuarios omitidos: '. $countedBad);
...
echo html_writer::tag('h2',"Duplicar cursos");
echo html_writer::tag('p',"Esta opcion es para duplicar todos los cursos semillas creados en el ambiente");
...
<div class="local-course-dev-form-container">
    <form method="post" >
        <div class="local-course-dev-form-controller-container">
            <button type="submit" name="buttonLoadCourses" id="buttonLoadCourses" class="local-course-dev-btn btn btn-info" >Duplicar Cursos</button>
        </div>
    </form>
</div>
<?php 
