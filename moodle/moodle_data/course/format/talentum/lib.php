...
class format_talentum extends format_topics 
{
    ...
    public function course_format_options($foreditform = false){
        ...
        if ($courseformatoptions === false) {
            ...
            $courseformatoptions['showdefaultsectionname'] = array(
                'default' => get_config('format_talentum', 'showdefaultsectionname'),
                'type' => PARAM_INT,
            );
            // $courseformatoptions['Modulevideohome'] = array(
            //     'default' => get_config('format_talentum', 'Modulevideohome'),
            //     'type' => PARAM_TEXT,
            // );
            $courseformatoptions['MainObjective'] = array(
                'default' => get_config('format_talentum', 'MainObjective'),
                'type' => PARAM_TEXT,
            );
            
            
            for ($i = 1; $i <= 4; $i++) {
                $divisortext = get_config('format_talentum', 'moduletitle'.$i);
                $divisortextvideo = get_config('format_talentum', 'modulevideo'.$i);
                $divisortextobj = get_config('format_talentum', 'moduleobjetive'.$i);
                $divisorname = get_config('format_talentum', 'modulename'.$i);
                if (!$divisortext) {
                    $divisortext = '';
                    $divisortextrule = '';
                    $divisorname = '';
            ...
            //     'label' => get_string('moduleimg', 'format_talentum'),
            //     'help' => 'moduleimg',
            //     'help_component' => 'format_talentum',
            ...
            //     file_prepare_draft_area($draftid_editor_file, $context->id, 'format_talentum', 'images', $i, $optionsfile);
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            ...
            $courseformatoptionsedit['numsections'] = array(
                'label' => get_string('numsections', 'format_talentum'),
                'element_type' => 'select',
            ...
            $courseformatoptionsedit['showdefaultsectionname'] = array(
                'label' => get_string('showdefaultsectionname', 'format_talentum'),
                'help' => 'showdefaultsectionname',
                'help_component' => 'format_talentum',
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        1 => get_string('yes', 'format_talentum'),
                        0 => get_string('no', 'format_talentum'),
                    ),
                ),
            );
            // $courseformatoptionsedit['Modulevideohome'] = array(
            //     'label' => get_string('Modulevideohome', 'format_talentum'),
            //     'help' => 'Modulevideohome',
            //     'help_component' => 'format_talentum',
            //     'element_type' => 'text',
            // );
            $courseformatoptionsedit['MainObjective'] = array(
                'label' => get_string('MainObjective', 'format_talentum'),
                'help' => 'MainObjective',
                'help_component' => 'format_talentum',
                'element_type' => 'text',
            );
            for ($i = 1; $i <= 4; $i++) {
                ...
                $courseformatoptionsedit['moduletitle'.$i] = array(
                    'label' => get_string('moduletitle', 'format_talentum', $i),
                    'help' => 'moduletitle',
                    'help_component' => 'format_talentum',
                    'element_type' => 'text',
                ...
                $courseformatoptionsedit['modulename'.$i] = array(
                    'label' => get_string('modulename', 'format_talentum', $i),
                    'help' => 'modulename',
                    'help_component' => 'format_talentum',
                    'element_type' => 'text',
                ); 
                ...
                //     'label' => get_string('modulevideo', 'format_talentum', $i),
                //     'help' => 'modulevideo',
                //     'help_component' => 'format_talentum',
                //     'element_type' => 'text',
                // );
                // $courseformatoptionsedit['moduleobjetive'.$i] = array(
                //     'label' => get_string('moduleobjetive', 'format_talentum', $i),
                //     'help' => 'moduleobjetive',
                //     'help_component' => 'format_talentum',
                //     'element_type' => 'text',
                // );
    ...
    //             $entry = file_save_draft_area_files($draftitemid, $context->id, 'format_talentum', 'images', $i, $filesoptions);
 ...
 */
function format_talentum_inplace_editable($itemtype, $itemid, $newvalue){
 ...
 */
// function format_talentum_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
...
//     $fs = get_file_storage();
//     $file = $fs->get_file($context->id, 'format_talentum', $filearea, $itemid, $filepath, $filename);
//     if (!$file) {
...
function progress_percentage($courseid) {
    global $DB;
    $activities = array();
    //get the id of all section
 ...
 */
function activities($course,$section,$user) {
    global $DB; 
    $completecount = 0;
    $complete_act = 0;
